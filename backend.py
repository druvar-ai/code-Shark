...
def encode_rain(rain_str: str) -> int:
    return {"DRY": 0, "MODERATE": 1, "WET": 2}.get(rain_str, 0)
...
def build_features(rain: str, water: int, vibration: str,
    ...
    r    = encode_rain(rain)
    w    = float(water)
    v    = 1.0 if vibration == "YES" else 0.0
...
def rule_based_risks(rain: str, water: int, vibration: str,
    ...
    # ── EARTHQUAKE RISK ─────────────────────────────────────────
    vib_active = 1.0 if vibration == "YES" else 0.0
...
def process_reading(rain: str, water: int, vibration: str):
    with _lock:
        ...
        if ml_pred is not None:
            # Blend: ML 55%, rules 45%
            overall = rule_overall * 0.45 + ml_pred * 0.55
        else:
...
def get_data():
    with _lock:
        stale = (time.time() - _state["last_update"]) > 6.0
        return {
            # Sensor values
            "rain":           _state["rain"],
            "water":          _state["water"],
            ...
            # Risk breakdown (%)
            "flood_risk":       _state["flood_risk"],
            "earthquake_risk":  _state["earthquake_risk"],
            "rain_risk":        _state["rain_risk"],
            "overall_risk":     _state["overall_risk"],

            # Timeline predictions (%)
            "future_10s":  _state["future_10s"],
            "future_30s":  _state["future_30s"],
            "future_60s":  _state["future_60s"],

            # Status
            "trend":         _state["trend"],
            "voice_message": _state["voice_message"],
            "ai_reasoning":  _state["ai_reasoning"],

            # Connection meta
            "connected":     _state["connected"],
            "stale":         stale,

            # Counters (for dashboard debug)
            "rain_duration":  _state["rain_counter"],
            "vib_duration":   _state["vib_counter"],
        }
